...
kinfer.cmd = function () {
  program
    .version("0.2.0")
    .description(
      "A CLI for transform azw3 and epub to mobi in windows platform"
    )
    .usage("-s [string]")
...
kinfer.init = async function () {
  ...
  if (!process.argv.slice(2).length) {
    console.log(
      chalk.red(figlet.textSync("kinfer-cli", { horizontalLayout: "full" }))
    );
    program.outputHelp();
    return;
  }
  ...
  if (!source) {
    console.log(
      chalk.red("Miss -s or --source param, using kinfer -h for help")
    );
    return;
  }
...
kinfer.transFile = async function (filename) {
  ...
  if (extname === ".azw3") {
    ...
    console.log(
      chalk.yellow(
        "The file cann't be resolved for it is not azw3 or epub format."
      )
    );
  }
};
...
kinfer.transEPUB = function (filename) {
  ...
  p1.on("close", () => {
    ...
    p2.on("close", () => {
      count++;
      console.log(
        chalk.green(`Compress mobi success, finish ${(count / total) * 100}%`)
      );
    });
  });
};
